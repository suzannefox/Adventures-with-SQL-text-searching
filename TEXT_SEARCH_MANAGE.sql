...
-- arguements are PART1 or PART3
DECLARE @SetupTables varchar(5)='INFO'

IF NOT @SetupTables IN ('PART1','PART3','PART4','INFO')
	RETURN
ELSE
	print('Processing '+ @setuptables)


IF @SetupTables = 'PART1'
BEGIN
	print 'PART 1 - DROP AND RECREATE TABLES'
	-- STEP 1
	...
	-- STEP 3
	print 'PART 2 - NOW IMPORT CSV DATA USING R'

	RETURN
END

IF @SetupTables = 'PART3'
BEGIN

	print 'PART 3 - ADD ID_IDENTITY COLUMN'

	-- STEP 4
	-- Add Id_Identity COLUMN to each table
	-- delivery_point
	...
	ALTER TABLE delivery_point
	ADD CONSTRAINT PK_delivery_point
	...
	ALTER TABLE blpu
	ADD CONSTRAINT PK_blpu
	PRIMARY KEY(Id_Identity)
END

IF @SetupTables = 'PART4'
BEGIN
	print 'PART 4 - CREATE INDEXES'
	-- STEP 5
	-- Drop and Create Text Catalog
	DROP FULLTEXT INDEX ON dbo.delivery_point
	DROP FULLTEXT INDEX ON dbo.blpu
	...
	-- STEP 6.2
	-- Create text index
	CREATE FULLTEXT INDEX ON dbo.delivery_point(ORGANISATION_NAME,
	                                            BUILDING_NAME,
	                                            SUB_BUILDING_NAME,
